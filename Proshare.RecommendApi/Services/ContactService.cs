...
namespace Proshare.RecommendApi.Services
{
    public class ContactService : IContactService
    {
        ...
        private readonly ILogger<ContactService> _ilogger;


        //服务请求地址
        private readonly string QueryAction = "/api/contacts/";

        public ContactService(IHttpClient httpClient, IDnsQuery dnsQuery, IOptions<ServiceDiscoveryOptions> option, ILogger<ContactService> logger)
        {
        ...
        {
            var apiAddress = await GetApplicateUrlFromConsulAsync();

            var result = await _httpClient.GetStringAsync(apiAddress + userId);

            return JsonConvert.DeserializeObject<List<Contact>>(result);
        }
        ...
        private async Task<string> GetApplicateUrlFromConsulAsync()
        {
            try
            {
                ...
                {
                    var result = await _dnsQuery.ResolveServiceAsync("service.consul", _serviceOption.DiscoverServiceName.Split(",")[1]);
